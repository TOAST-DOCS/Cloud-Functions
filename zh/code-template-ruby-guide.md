...
```
**Note**: the `Gemfile.lock` file is generated by running `bundle install` and is recommended to be compressed as it locks in the exact versions of your dependencies.
...
### Cautions for upload
- **Upload File**: Upload **ZIP file** which includes `*.rb`, `Gemfile`, and `Gemfile.lock`.
...
gem "httparty", "~> 0.20.0" # HTTP client
```
...
### Caution
- **Bundler version**: We recommend using Bundler 1.x (1.17.3 or later) when generating `Gemfile.lock`.
